RSpec::Matchers.define :contain do |*expected_contents|
  ...
  failure_message do |file_path|
    "expected the file #{file_path} to contain " +
      if expected_contents.size > 1
        "#{expected_contents.map(&:inspect).to_sentence} but " \
          "#{not_found_expectations.map(&:inspect).to_sentence} " \
            "#{not_found_expectations.size > 1 ? 'were' : 'was'} not found"
      else
        "#{expected_contents.first.inspect} " \
          "but it contained #{@actual_contents.inspect}"
      end
  end
  ...
  failure_message_when_negated do |file_path|
    "expected the file #{file_path} to not contain " +
      if expected_contents.size > 1
        "#{expected_contents.map(&:inspect).to_sentence} but " \
          "#{found_expectations.map(&:inspect).to_sentence} " \
            "#{found_expectations.size > 1 ? 'were' : 'was'} found"
      else
